...
const fetchUsers = (query) => {
    ...
    fetch(apiUrl + query).then((response)=>{
...
const display = (users) => {
    ...
    users?.map((user, i)=>{
        str += `  
    <div id="list"class="card shadow-lg" style="width: 18rem">
        <img
        ...
        <p class="card-text">
            <ul class="list-unstyled">
                ...
                <li><i class="fa-solid fa-map-location-dot"></i>${user.location.street.number} ${user.location.city} ${user.location.country} ${user.location.postcode}</li>
            </ul>
        </p>
        </div>
    </div>`
